    ...
    var effect = {
        ...
        obtain: function (event) {
            ...
            if (config.renderMode) {
                ...
                //给选中的级-添加值和文字
                $parent.siblings('.reveal').removeClass('df-color forbid').text(name).siblings('.input-price').val(name);
    ...
    Citypicker.prototype = {
        ...
        setCityVal: function (val) {
            ...
            $.each(arrayVal, function (key, value) {
                ...
                if (config.renderMode) {
                    $original.find('.reveal').text(value.name).removeClass('df-color forbid').siblings('.input-price').val(value.name);
                    ...
                    $original.find('.caller[value="'+value.name+'"]').prop('selected', true);
                }
